...
class ReportingPeriod(ActivityLogMixin, BaseModel):
    ...
    def clean(self):
        ...
        if self.parent and self.start_date and self.end_date:
            parent = self.parent

            if parent.start_date and self.start_date < parent.start_date:
                errors["start_date"] = (
                    "Child period start date must not be before parent start date."
                )

            if parent.end_date and self.end_date > parent.end_date:
                errors["end_date"] = (
                    "Child period end date must not be after parent end date."
                )
        ...
        if (
            ...
            qs = ReportingPeriod.objects.filter(
            ...
            if overlap_exists:
                errors["period_type"] = (
                    "An annual reporting period overlaps with an existing annual period."
                )
