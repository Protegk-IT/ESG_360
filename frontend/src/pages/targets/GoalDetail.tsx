...
export default function GoalDetail() {
  ...
  const canManage = Boolean(user?.is_superuser || permissions.includes("target.set"));
  const { id = "" } = useParams();
  ...
  return <AppShell title={goal?.name ?? "Goal"} description="KPI target planning and approved-data progress.">
    <main className="mx-auto w-full max-w-7xl space-y-4 px-0 pb-6">
      {loading ? <DetailSkeleton /> : error ? <Card><CardContent className="py-14 text-center text-destructive"><CircleAlert className="mx-auto mb-3 size-7" />{error}</CardContent></Card> : <>
        ...
        {kpis.length ? <>
          ...
          <section className="grid gap-4 lg:grid-cols-[minmax(280px,0.72fr)_minmax(0,1.8fr)]"><BaselinePanel target={activeTarget} canManage={canManage} onConfigure={() => openTarget(activeTarget)} /><ProgressPanel kpi={selectedKpi} target={activeTarget} graph={graph} progress={progress} /></section>
        </> : <Card className="border-dashed"><CardContent className="flex min-h-64 flex-col items-center justify-center p-8 text-center"><span className="mb-4 grid size-12 place-items-center rounded-full bg-primary/10"><TargetIcon className="size-6 text-primary" /></span><h2 className="text-lg font-semibold">{canManage ? "Add a KPI to begin planning" : "No KPIs are available in your target scope"}</h2><p className="mt-2 max-w-md text-sm text-muted-foreground">{canManage ? "Choose an active canonical M4 datapoint. Its approved M5 values will become the KPI’s actuals." : "Ask a target setter to configure a KPI or target in an OrgNode you can view."}</p>{canManage && <Button className="mt-5" onClick={() => setKpiOpen(true)}><Plus className="mr-2 size-4" />Add KPI</Button>}</CardContent></Card>}
      </>}
    </main>
    ...
    <InitiativesDialog open={initiativesOpen} onOpenChange={setInitiativesDialogOpen} kpi={selectedKpi} initiatives={initiatives} users={users} orgNodes={orgNodes} canManage={canManage} editing={editingInitiative} formOpen={initiativeFormOpen} form={initiativeForm} saving={saving} onEdit={openInitiative} onChange={setInitiativeForm} onSave={() => void saveInitiative()} />
  </AppShell>;
}
