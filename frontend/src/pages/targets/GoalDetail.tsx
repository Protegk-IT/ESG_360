...
type KpiForm = { code: string; name: string; description: string; metric_source_type: "DATAPOINT" | "MANUAL_REFERENCE"; datapoint: string; metric_code: string; direction: Direction; aggregation: "SUM" | "AVG" | "LATEST" | "COUNT" | "NONE" };
type TargetForm = { org_node: string; baseline_period: string; baseline_value: string; baseline_unit: string; baseline_source: "SYSTEM_DATA" | "REFERENCE"; target_period: string; target_value: string; target_unit: string; target_type: "ABSOLUTE" | "INTENSITY" | "PERCENTAGE"; owner: string; basis: string; rationale: string; methodology: string; status: string; source_reference: string };
...
const emptyKpi: KpiForm = { code: "", name: "", description: "", metric_source_type: "DATAPOINT", datapoint: "", metric_code: "", direction: "REDUCE", aggregation: "SUM" };
const emptyTarget: TargetForm = { org_node: "none", baseline_period: "", baseline_value: "", baseline_unit: "", baseline_source: "REFERENCE", target_period: "", target_value: "", target_unit: "", target_type: "ABSOLUTE", owner: "none", basis: "OTHER", rationale: "", methodology: "", status: "DRAFT", source_reference: "" };
...
export default function GoalDetail() {
  ...
  const canManageGoal = Boolean(goal?.can_manage);
  const canManageKpi = Boolean(selectedKpi?.can_manage);
  const canManageTarget = Boolean(activeTarget?.can_manage ?? selectedKpi?.can_manage);
  ...
  function openTarget(target?: Target) {
    ...
    setTargetForm(target ? { org_node: target.org_node ?? "none", baseline_period: target.baseline_period, baseline_value: editableDecimal(target.baseline_value), baseline_unit: target.baseline_unit ?? "", baseline_source: target.baseline_source, target_period: target.target_period, target_value: editableDecimal(target.target_value), target_unit: target.target_unit ?? "", target_type: target.target_type, owner: target.owner ?? "none", basis: target.basis, rationale: target.rationale, methodology: target.methodology, status: target.status, source_reference: target.source_reference ?? "" } : { ...emptyTarget, baseline_unit: selectedKpi?.default_unit ?? "", target_unit: selectedKpi?.default_unit ?? "" });
    setTargetOpen(true);
  }
  ...
  async function saveKpi() {
    if (!kpiForm.code || !kpiForm.name || (kpiForm.metric_source_type === "DATAPOINT" && !kpiForm.datapoint) || (kpiForm.metric_source_type === "MANUAL_REFERENCE" && !kpiForm.metric_code)) return;
    try { setSaving(true); const payload = { ...kpiForm, datapoint: kpiForm.metric_source_type === "DATAPOINT" ? kpiForm.datapoint : null, metric_code: kpiForm.metric_source_type === "MANUAL_REFERENCE" ? kpiForm.metric_code : "" }; if (editingKpi) await TargetsApi.updateKpi(editingKpi.id, payload); else await TargetsApi.createKpi(id, payload); setKpiOpen(false); setEditingKpi(null); setKpiForm(emptyKpi); await loadGoal(); toast.success(editingKpi ? "KPI updated" : "KPI added"); }
    catch { toast.error("Unable to add KPI. Review the canonical datapoint and try again."); }
    finally { setSaving(false); }
  }
  async function saveTarget() {
    if (!selected || !targetForm.baseline_period || !targetForm.target_period || !targetForm.target_value || (targetForm.baseline_source === "REFERENCE" && !targetForm.baseline_value)) return;
    try {
      setSaving(true);
      const payload = { ...targetForm, org_node: targetForm.org_node === "none" ? null : targetForm.org_node, owner: targetForm.owner === "none" ? null : targetForm.owner, baseline_unit: targetForm.baseline_unit || null, target_unit: targetForm.target_unit || null, baseline_value: targetForm.baseline_value || undefined };
  ...
  async function saveGoal(form: GoalForm) {
    ...
    try {
      ...
      const legacyCompany = goal.company ? undefined : (await CompanyApi.getProfile()).data.id;
      const updated = await TargetsApi.updateGoal(goal.id, { ...payload, name: form.name.trim(), description: form.description.trim(), material_topic: form.material_topic || null, material_subtopic: form.material_subtopic || null, source_assessment_topic: form.source_assessment_topic || null, owner: form.owner || null, ...(legacyCompany ? { company: legacyCompany } : {}) });
  ...
  return <AppShell title={goal?.name ?? "Goal"} description="KPI target planning and approved-data progress.">
    <main className="mx-auto w-full max-w-7xl space-y-4 px-0 pb-6">
      {loading ? <DetailSkeleton /> : error ? <Card><CardContent className="py-14 text-center text-destructive"><CircleAlert className="mx-auto mb-3 size-7" />{error}</CardContent></Card> : <>
        ...
        <section className="grid gap-3 md:grid-cols-3"><MetricCard title="Actual" value={actual?.actual_value ?? "No approved data"} unit={activeTarget?.baseline_unit_code ?? selectedKpi?.default_unit_code} subtitle={actual ? "Latest approved M5 value" : "Only approved M5 submissions appear here"} icon={<LineChartIcon className="size-4" />} /><MetricCard title="Target trajectory" value={activeTarget ? `${formatMetric(activeTarget.baseline_value)} → ${formatMetric(activeTarget.target_value)}` : "Not configured"} unit={activeTarget?.baseline_unit_code} subtitle="Straight-line plan between baseline and endpoint" icon={<TargetIcon className="size-4" />} /><MetricCard title="Target" value={activeTarget?.target_value ?? "Not configured"} unit={activeTarget?.target_unit_code} subtitle={activeTarget?.target_period_name ?? "Configure target to begin"} icon={<TargetIcon className="size-4" />} /></section>
          {canManageKpi && selectedKpi && <div className="flex justify-end"><Button size="sm" variant="outline" onClick={() => { setEditingKpi(selectedKpi); setKpiForm({ code: selectedKpi.code, name: selectedKpi.name, description: selectedKpi.description, metric_source_type: selectedKpi.metric_source_type === "MANUAL_REFERENCE" ? "MANUAL_REFERENCE" : "DATAPOINT", datapoint: selectedKpi.datapoint ?? "", metric_code: selectedKpi.metric_code ?? "", direction: selectedKpi.direction, aggregation: selectedKpi.aggregation }); setKpiOpen(true); }}><Pencil className="mr-2 size-3.5" />Edit KPI</Button></div>}
          <section className="grid gap-4 lg:grid-cols-[minmax(280px,0.72fr)_minmax(0,1.8fr)]"><BaselinePanel target={activeTarget} canManage={canManageTarget} onConfigure={() => openTarget(activeTarget)} /><ProgressPanel kpi={selectedKpi} target={activeTarget} graph={graph} progress={progress} /></section>
        </> : <Card className="border-dashed"><CardContent className="flex min-h-64 flex-col items-center justify-center p-8 text-center"><span className="mb-4 grid size-12 place-items-center rounded-full bg-primary/10"><TargetIcon className="size-6 text-primary" /></span><h2 className="text-lg font-semibold">{canManageGoal ? "Add a KPI to begin planning" : "No KPIs are available in your target scope"}</h2><p className="mt-2 max-w-md text-sm text-muted-foreground">{canManageGoal ? "Choose an active canonical M4 datapoint. Its approved M5 values will become the KPI’s actuals." : "Ask a target setter to configure a KPI or target in an OrgNode you can view."}</p>{canManageGoal && <Button className="mt-5" onClick={() => setKpiOpen(true)}><Plus className="mr-2 size-4" />Add KPI</Button>}</CardContent></Card>}
      </>}
    </main>
    <Dialog open={kpiOpen} onOpenChange={setKpiOpen}><DialogContent className="max-h-[90vh] gap-3 overflow-y-auto p-5 sm:max-w-xl"><DialogHeader className="pr-8"><DialogTitle className="text-lg font-semibold">{editingKpi ? "Edit KPI" : "Add KPI"}</DialogTitle></DialogHeader><div className="space-y-3"><Field label="Metric source"><Select value={kpiForm.metric_source_type} onValueChange={(value) => setKpiForm({ ...kpiForm, metric_source_type: value as KpiForm["metric_source_type"], datapoint: "" })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="DATAPOINT">Canonical M4 datapoint</SelectItem><SelectItem value="MANUAL_REFERENCE">Manual reference</SelectItem></SelectContent></Select></Field>{kpiForm.metric_source_type === "DATAPOINT" ? <Field label="Canonical M4 datapoint"><Select value={kpiForm.datapoint} onValueChange={selectDatapoint}><SelectTrigger><SelectValue placeholder="Select a numeric datapoint" /></SelectTrigger><SelectContent>{datapoints.map((datapoint) => <SelectItem key={datapoint.id} value={datapoint.id}>{datapoint.label} · {datapoint.code}</SelectItem>)}</SelectContent></Select></Field> : <Field label="Governed metric reference"><Input value={kpiForm.metric_code} onChange={(event) => setKpiForm({ ...kpiForm, metric_code: event.target.value })} placeholder="e.g. policy.water-risk-index" /></Field>}<div className="grid gap-3 sm:grid-cols-2"><Field label="KPI name"><Input value={kpiForm.name} onChange={(event) => setKpiForm({ ...kpiForm, name: event.target.value })} /></Field><Field label="Stable KPI code"><Input value={kpiForm.code} onChange={(event) => setKpiForm({ ...kpiForm, code: event.target.value })} /></Field></div><div className="grid gap-3 sm:grid-cols-2"><Field label="Direction"><Select value={kpiForm.direction} onValueChange={(value) => setKpiForm({ ...kpiForm, direction: value as Direction })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="REDUCE">Reduce</SelectItem><SelectItem value="INCREASE">Increase</SelectItem><SelectItem value="MAINTAIN">Maintain</SelectItem></SelectContent></Select></Field><Field label="Aggregation"><Select value={kpiForm.aggregation} onValueChange={(value) => setKpiForm({ ...kpiForm, aggregation: value as KpiForm["aggregation"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="SUM">Sum approved values</SelectItem><SelectItem value="AVG">Average approved values</SelectItem><SelectItem value="LATEST">Latest approved value</SelectItem><SelectItem value="COUNT">Count approved answers</SelectItem><SelectItem value="NONE">One scoped value</SelectItem></SelectContent></Select></Field></div><Field label="Description"><Textarea className="min-h-18" value={kpiForm.description} onChange={(event) => setKpiForm({ ...kpiForm, description: event.target.value })} /></Field></div><DialogFooter className="-mx-5 -mb-5 px-5 py-3"><Button variant="outline" onClick={() => setKpiOpen(false)}>Cancel</Button><Button disabled={saving || !kpiForm.name || !kpiForm.code || (kpiForm.metric_source_type === "DATAPOINT" ? !kpiForm.datapoint : !kpiForm.metric_code)} onClick={() => void saveKpi()}>{saving ? "Saving…" : editingKpi ? "Save KPI" : "Add KPI"}</Button></DialogFooter></DialogContent></Dialog>
    <Dialog open={targetOpen} onOpenChange={setTargetOpen}><DialogContent className="max-h-[90vh] gap-3 overflow-y-auto p-5 sm:max-w-xl"><DialogHeader className="pr-8"><DialogTitle className="text-lg font-semibold">{editingTarget ? "Edit target" : "Configure target"}</DialogTitle></DialogHeader><TargetFields form={targetForm} periods={periods} units={applicableUnits} orgNodes={orgNodes} users={users} onChange={setTargetForm} /><DialogFooter className="-mx-5 -mb-5 px-5 py-3"><Button variant="outline" onClick={() => setTargetOpen(false)}>Cancel</Button><Button disabled={saving || !targetForm.baseline_period || !targetForm.target_period || !targetForm.target_value || (targetForm.baseline_source === "REFERENCE" && !targetForm.baseline_value)} onClick={() => void saveTarget()}>{saving ? "Saving…" : editingTarget ? "Save target" : "Configure target"}</Button></DialogFooter></DialogContent></Dialog>
    ...
    <InitiativesDialog open={initiativesOpen} onOpenChange={setInitiativesDialogOpen} kpi={selectedKpi} initiatives={initiatives} users={users} orgNodes={orgNodes} canManage={canManageKpi} editing={editingInitiative} formOpen={initiativeFormOpen} form={initiativeForm} saving={saving} onEdit={openInitiative} onChange={setInitiativeForm} onSave={() => void saveInitiative()} />
  </AppShell>;
}
...
function BaselinePanel({ target, canManage, onConfigure }: { target?: Target; canManage: boolean; onConfigure: () => void }) { return <Card size="sm" className="border-[#D9DEE8] shadow-none"><CardHeader className="border-b-0 px-4 py-4"><CardTitle className="text-base">Baseline & target</CardTitle><CardDescription className="mt-1 text-xs leading-5">Where this KPI starts, why the target was set, and where it should reach.</CardDescription></CardHeader><CardContent className="space-y-4 px-4 py-0 pb-4">{target ? <dl className="space-y-3 text-sm"><div><dt className="text-xs text-muted-foreground">Baseline</dt><dd className="mt-0.5 font-medium">{formatMetric(target.baseline_value)} {target.baseline_unit_code} · {target.baseline_period_name}</dd><p className="mt-0.5 text-xs text-muted-foreground">{target.baseline_source === "SYSTEM_DATA" ? "Frozen from approved system data" : "Reference/manual baseline"}</p></div><div><dt className="text-xs text-muted-foreground">Target endpoint</dt><dd className="mt-0.5 font-medium">{formatMetric(target.target_value)} {target.target_unit_code} · {target.target_period_name}</dd></div><div><dt className="text-xs text-muted-foreground">Basis</dt><dd className="mt-0.5">{target.basis.replaceAll("_", " ").toLowerCase()}</dd></div>{target.org_node_name && <div><dt className="text-xs text-muted-foreground">Scope</dt><dd className="mt-0.5">{target.org_node_name}</dd></div>}</dl> : <p className="text-sm leading-5 text-muted-foreground">Set a frozen baseline and an endpoint to make this KPI’s target trajectory visible.</p>}{canManage && <Button size="sm" className="w-full" variant={target ? "outline" : "default"} onClick={onConfigure}>{target ? "Edit target configuration" : "Configure target"}</Button>}</CardContent></Card>; }
function ProgressPanel({ kpi, target, graph, progress }: { kpi?: KPI; target?: Target; graph: Array<{ name: string; trajectory?: number; actual?: number; target?: number }>; progress?: TargetProgress }) { return <Card size="sm" className="border-[#D9DEE8] shadow-none"><CardHeader className="px-4 py-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="text-base">{kpi?.name ?? "KPI"} progress</CardTitle><CardDescription className="mt-1 text-xs leading-5">Approved actuals and a deterministic target trajectory by reporting period.</CardDescription></div>{target && <Badge variant="info">{target.target_type.toLowerCase()}</Badge>}</div></CardHeader><CardContent className="h-[300px] px-3 py-2 sm:px-4">{graph.length ? <ResponsiveContainer width="100%" height="100%" minWidth={1}><LineChart data={graph} margin={{ top: 16, right: 12, left: 2, bottom: 4 }}><CartesianGrid vertical={false} stroke="#ECEEF5" /><XAxis dataKey="name" tick={{ fontSize: 11, fill: "#6B7280" }} axisLine={{ stroke: "#D9DEE8" }} tickLine={false} /><YAxis tick={{ fontSize: 11, fill: "#6B7280" }} tickFormatter={formatMetric} axisLine={false} tickLine={false} width={64} /><Tooltip contentStyle={{ border: "1px solid #D9DEE8", borderRadius: 8, boxShadow: "0 4px 12px rgba(30, 33, 48, 0.12)" }} formatter={(value: number | string | undefined) => value == null ? "No data" : formatMetric(value)} /><Line type="linear" dataKey="trajectory" name="Target trajectory" stroke="#4A3FD6" strokeWidth={2.5} dot={{ r: 3, fill: "#FFFFFF", strokeWidth: 2 }} connectNulls /><Line type="monotone" dataKey="actual" name="Approved actual" stroke="#22C55E" strokeWidth={2.5} dot={{ r: 3.5, fill: "#FFFFFF", strokeWidth: 2 }} connectNulls /><Line type="linear" dataKey="target" name="Target endpoint" stroke="#4A3FD6" strokeWidth={0} dot={{ r: 4.5, fill: "#FFFFFF", stroke: "#4A3FD6", strokeWidth: 2.5 }} /></LineChart></ResponsiveContainer> : <div className="grid h-full place-items-center text-center"><div><LineChartIcon className="mx-auto mb-2 size-6 text-muted-foreground" /><p className="font-medium">No target trajectory yet</p><p className="mt-1 text-sm text-muted-foreground">Configure a baseline and target to show the planned path. Approved actuals will appear as they are collected.</p></div></div>}</CardContent>{progress && !progress.progress.some((row) => row.actual_value != null) && <div className="border-t border-[#ECEEF5] px-4 py-2 text-xs text-muted-foreground">No approved M5 data is available in this target window yet. Values are never shown as zero.</div>}</Card>; }
function TargetFields({ form, periods, units, orgNodes, users, onChange }: { form: TargetForm; periods: ReportingPeriod[]; units: Unit[]; orgNodes: OrgNode[]; users: UserData[]; onChange: (value: TargetForm) => void }) {
  const update = (key: keyof TargetForm, value: string) => onChange({ ...form, [key]: value });
  return <div className="space-y-3">
    <div className="grid gap-3 sm:grid-cols-2"><Field label="Target scope"><Select value={form.org_node} onValueChange={(value) => update("org_node", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Company-wide</SelectItem>{orgNodes.map((node) => <SelectItem key={node.id} value={node.id}>{node.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Target type"><Select value={form.target_type} onValueChange={(value) => update("target_type", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ABSOLUTE">Absolute</SelectItem><SelectItem value="INTENSITY">Intensity</SelectItem><SelectItem value="PERCENTAGE">Percentage</SelectItem></SelectContent></Select></Field></div>
    <div className="grid gap-3 sm:grid-cols-2"><Field label="Baseline source"><Select value={form.baseline_source} onValueChange={(value) => update("baseline_source", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="REFERENCE">Reference/manual</SelectItem><SelectItem value="SYSTEM_DATA">Approved system data</SelectItem></SelectContent></Select></Field><Field label="Target owner"><Select value={form.owner} onValueChange={(value) => update("owner", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">No owner assigned</SelectItem>{users.map((user) => <SelectItem key={user.id} value={String(user.id)}>{ownerLabel(user)}</SelectItem>)}</SelectContent></Select></Field></div>
    <div className="grid gap-3 sm:grid-cols-2"><Field label="Baseline reporting period"><Select value={form.baseline_period} onValueChange={(value) => update("baseline_period", value)}><SelectTrigger><SelectValue placeholder="Choose annual baseline" /></SelectTrigger><SelectContent>{periods.map((period) => <SelectItem key={period.id} value={period.id}>{period.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Baseline value"><Input disabled={form.baseline_source === "SYSTEM_DATA"} inputMode="decimal" value={form.baseline_value} onChange={(event) => update("baseline_value", event.target.value)} placeholder={form.baseline_source === "SYSTEM_DATA" ? "Resolved from approved M5 data" : "e.g. 120000"} /></Field><Field label="Target reporting period"><Select value={form.target_period} onValueChange={(value) => update("target_period", value)}><SelectTrigger><SelectValue placeholder="Choose annual target" /></SelectTrigger><SelectContent>{periods.map((period) => <SelectItem key={period.id} value={period.id}>{period.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Target value"><Input inputMode="decimal" value={form.target_value} onChange={(event) => update("target_value", event.target.value)} placeholder="e.g. 90000" /></Field></div>
    <div className="grid gap-3 sm:grid-cols-2"><Field label="Unit"><Select value={form.baseline_unit} onValueChange={(value) => onChange({ ...form, baseline_unit: value, target_unit: value })}><SelectTrigger><SelectValue placeholder="Choose unit" /></SelectTrigger><SelectContent>{units.map((unit) => <SelectItem key={unit.id} value={unit.id}>{unit.name} ({unit.code})</SelectItem>)}</SelectContent></Select></Field><Field label="Target status"><Select value={form.status} onValueChange={(value) => update("status", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["DRAFT", "ACTIVE", "ACHIEVED", "MISSED", "RETIRED"].map((status) => <SelectItem key={status} value={status}>{status.toLowerCase()}</SelectItem>)}</SelectContent></Select></Field></div>
    <Field label="Target basis"><Select value={form.basis} onValueChange={(value) => update("basis", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{targetBasis.map((basis) => <SelectItem key={basis} value={basis}>{basis.replaceAll("_", " ").toLowerCase()}</SelectItem>)}</SelectContent></Select></Field><Field label="Source / provenance"><Input value={form.source_reference} onChange={(event) => update("source_reference", event.target.value)} placeholder="Evidence or reference for the baseline/target" /></Field><Field label="Rationale"><Textarea className="min-h-18" value={form.rationale} onChange={(event) => update("rationale", event.target.value)} placeholder="Why is this endpoint appropriate?" /></Field><Field label="Methodology"><Textarea className="min-h-18" value={form.methodology} onChange={(event) => update("methodology", event.target.value)} placeholder="Optional target methodology or assumptions" /></Field></div>;
}
